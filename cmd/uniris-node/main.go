...
func main() {
	...
	os.Setenv("PLUGINS_DIR", filepath.Join(dir, "../../pkg/plugins"))
	startTransactionServer()
}

func startTransactionServer() {
	p, err := plugin.Open(filepath.Join(os.Getenv("PLUGINS_DIR"), "transactionService/plugin.so"))
	if err != nil {
		log.Fatal(err)
	}

	sym, err := p.Lookup("StartServer")
	if err != nil {
		log.Fatal(err)
	}

	f := sym.(func(int, interface{}, interface{}, interface{}, interface{}) error)
	if err := f(3000, nil, nil, nil, nil); err != nil {
		log.Printf("error: %s\n", err.Error())
	}
}
