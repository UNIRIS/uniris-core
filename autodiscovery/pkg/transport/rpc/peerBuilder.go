...
func (f PeerBuilder) FromPeerDiscovered(p *api.PeerDiscovered) discovery.Peer {
	...
	id := discovery.NewPeerIdentity(net.ParseIP(p.Identity.IP), p.Identity.Port, p.Identity.PublicKey)
	hb := discovery.NewPeerHeartbeatState(time.Unix(p.HeartbeatState.GenerationTime, 0), p.HeartbeatState.ElapsedHeartbeats)
