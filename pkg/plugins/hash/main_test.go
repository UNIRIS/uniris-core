...
func TestNewVersionnedHash(t *testing.T) {
	...
	vh := versionHash(crypto.SHA256, h)
	assert.Equal(t, crypto.SHA256, HashAlgorithm(vh))
	assert.Equal(t, h, HashDigest(vh))
}
...
func TestHashBytes(t *testing.T) {
	...
	h.Write([]byte("hello"))
	hash := h.Sum(nil)
	assert.Equal(t, versionHash(DefaultHashAlgo, hash), vHash)
}
...
func TestIsValidHashWithEmptyDigest(t *testing.T) {
	vh := versionHash(crypto.SHA256, []byte(""))
	assert.False(t, IsValidHash(vh))
}
...
func TestIsValidHashWithInvalidSize(t *testing.T) {
	vh := versionHash(crypto.SHA256, []byte("abc"))
	assert.False(t, IsValidHash(vh))
}
