...
// - Requests storage
func LeadMining(tx chain.Transaction, minValids int, wHeaders []chain.NodeHeader, poolR PoolRequester, nodePub crypto.PublicKey, nodePv crypto.PrivateKey, sharedKeyReader shared.KeyReader) error {
	...
	go func() {
		minedTx, err := mineTransaction(tx, wHeaders, sPool, lastVPool, minValids, nodePub, nodePv, sharedKeyReader, poolR)
		if err != nil {
...
func mineTransaction(tx chain.Transaction, wHeaders []chain.NodeHeader, sPool Pool, lastVPool Pool, minValids int, nodePub crypto.PublicKey, nodePv crypto.PrivateKey, sharedKeyReader shared.KeyReader, poolR PoolRequester) (chain.Transaction, error) {
