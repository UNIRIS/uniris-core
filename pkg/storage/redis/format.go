...
func FormatSeedToHash(seed discovery.PeerIdentity) map[string]interface{} {
	return map[string]interface{}{
		"ip":        seed.IP().String(),
...
func FormatHashToSeed(hash map[string]string) discovery.PeerIdentity {
	port, _ := strconv.Atoi(hash["port"])
	return discovery.NewPeerIdentity(
		net.ParseIP(hash["ip"]),
		port,
		hash["publicKey"],
	)
