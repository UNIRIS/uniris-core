...
type Notifier interface {
	NotifyReachable(Peer) error
	NotifyUnreachable(Peer) error
...
func (s Service) Gossip(localPeer Peer, seeds []Seed, ticker *time.Ticker) (abortChan chan error, err error) {
...
func (s Service) startCycle(localP Peer, seeds []Seed, dChan chan<- Peer, rChan chan<- Peer, uChan chan<- Peer, eChan chan<- error) {
...
func (s Service) refreshLocalPeer(p Peer, sp []Seed) error {
...
func (s Service) handleReachables(c cycle, rChan chan<- Peer, eChan chan<- error) {
...
func (s Service) handleUnreachables(c cycle, uChan chan<- Peer, eChan chan<- error) {
...
//SynchronizeGossip compare the peers received from the SYN gossip request
// and returns the unknown peers and the peers the sender does not known
func (s Service) SynchronizeGossip(receivedPeers []Peer) ([]Peer, []Peer, error) {
	kp, err := s.repo.ListKnownPeers()
	if err != nil {
		return nil, nil, err
	}
...
//AcknowledgeGossip store the receiving peers from the ACK gossip request
func (s Service) AcknowledgeGossip(receivedPeers []Peer) error {
